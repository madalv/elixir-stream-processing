defmodule Week6.PoolManager do
  use GenServer
  require Logger
  ...
  def handle_cast({:increase}, state) do
    Week6.GenericSupervisor.add_worker(state[:sup_pid], state[:module], state[:lb_pid])
    {:noreply, state}
  end

  def handle_cast({:decrease}, state) do
    nr = Week6.GenericSupervisor.get_workers_len(state[:sup_pid])

    if nr > state[:min_nodes],
      do:
        Week6.GenericSupervisor.remove_last_worker(
          state[:sup_pid],
          state[:module],
          state[:lb_pid]
        )
