defmodule Week6.GenericSupervisor do
  use Supervisor
  require Logger
  ...
  def init({nr, module, init_state}) do
    ...
    {:ok, lb} = Week6.LoadBalancer.start_link({nr, self(), module})

    {:ok, pool_manager} =
      Week6.PoolManager.start_link(%{
        min_nodes: 3,
        req_bound_up: 40,
        req_bound_down: 30,
        sup_pid: self(),
    ...
    Week6.LoadBalancer.add_pool_manager_pid(lb, pool_manager)
  ...
  def add_worker(pid, module, lb_pid) do
    ...
    Week6.LoadBalancer.add_new_node(lb_pid, nr)
  ...
  def remove_last_worker(pid, module, lb_pid) do
    ...
    Week6.LoadBalancer.remove_node(lb_pid, nr)
