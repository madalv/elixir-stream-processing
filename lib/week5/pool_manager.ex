defmodule Week5.PoolManager do
  use GenServer
  require Logger
  ...
  def handle_cast({:increase}, state) do
    Week5.GenericSupervisor.add_worker(state[:sup_pid], state[:module], state[:lb_pid])
    {:noreply, state}
  end

  def handle_cast({:decrease}, state) do
    nr = Week5.GenericSupervisor.get_workers_len(state[:sup_pid])

    if nr > state[:min_nodes],
      do:
        Week5.GenericSupervisor.remove_last_worker(
          state[:sup_pid],
          state[:module],
          state[:lb_pid]
        )
